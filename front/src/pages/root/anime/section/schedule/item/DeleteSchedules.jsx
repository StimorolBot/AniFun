...
export const DeleteSchedules = memo(
	({ isShowAlert, setUpdateAlert, setAlertData }) => {
		...
		const { isFetching, refetch, error } = useQuery({
			queryKey: [rootAnimeSchedule.delete],
			retry: 1,
			enabled: false,
			queryFn: async () => {
				return api
					.delete("/admin/anime/delete-schedules", {
		...
		return (
			<form
				...
				id="root-anime-form-delete-schedule"
				onSubmit={handleSubmit(onSubmit)}
			>
				<h2 className="root-anime__title">Удалить расписание</h2>
				<InputTitle
					id={"root-schedule-delete"}
				...
				<div className="root-anime__btn-container">
					{isFetching ? (
						...
						<BtnDefault
							type="submit"
							form="root-anime-form-delete-schedule"
