...
from src.app.auth.oauth.api_v1.discord.config import \
    settings as discord_settings
from src.app.auth.oauth.api_v1.google.config import settings as google_settings
from src.app.auth.oauth.api_v1.google.oauth import oauth
from src.app.auth.oauth.api_v1.telegram.schemas import OauthTelegram
from src.app.auth.utils.v1.create_user import create_user
from src.database.session import get_async_session
from src.utils.crud import crud
from src.utils.logger import auth_log

oauth_router = APIRouter(prefix="/auth/oauth2", tags=["auth-oauth2"])


@oauth_router.get("/uri-google", status_code=status.HTTP_200_OK, summary="Получить google uri")
async def get_redirect_uri_google(request: Request) -> str:
...
@oauth_router.get("/auth-google", summary="Авторизация через google")
async def auth_google(request: Request, session: AsyncSession = Depends(get_async_session)):
    try:
        ...
        tokens = jwt_token.create_tokens(sub=data_table.uuid, auth_type=AuthType.GOOGLE.value)
...
@oauth_router.get("/uri-discord", status_code=status.HTTP_200_OK, summary="Получить discord uri")
async def get_redirect_uri_discord():
    return discord_settings.GET_AUTH_URI


@oauth_router.get("/auth-discord", summary="Авторизация через discord")
...
@oauth_router.post("/auth-telegram", summary="Авторизация через telegram")
