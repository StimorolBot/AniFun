...
class AnimeTable(Base):
    ...
    type: Mapped[str] = mapped_column(ForeignKey("type_sub_table.label"))
    season: Mapped[str] = mapped_column(ForeignKey("season_sub_table.label"))
    age_restrict: Mapped[str] = mapped_column(ForeignKey("age_r_sub_table.label"))
    status: Mapped[str] = mapped_column(ForeignKey("status_sub_table.label"))
    ...
    poster_rs: Mapped["PosterTable"] = relationship(back_populates="anime_rs", cascade="all, delete-orphan",
                                                    uselist=False)
    banner_rs: Mapped["BannerTable"] = relationship(back_populates="anime_rs", cascade="all, delete-orphan",
                                                    uselist=False)
